...
function StarLayer({ count = 1100, depth = 140, speed = 7, size = 90, seed = 0 }: StarLayerProps) {
  ...
  useFrame((state, delta) => {
    ...
    const scrollOffset = (typeof window !== 'undefined' ? window.scrollY : 0) * 0.002
...
function Starfield({ isMobile }: { isMobile: boolean }) {
  const m = isMobile ? 0.45 : 1.0
  return (
    <group>
      <StarLayer count={Math.max(200, Math.floor(900 * m))} depth={140} speed={6} size={90} seed={1} />
      <StarLayer count={Math.max(150, Math.floor(700 * m))} depth={180} speed={10} size={130} seed={2} />
      <StarLayer count={Math.max(100, Math.floor(500 * m))} depth={220} speed={4} size={150} seed={3} />
    </group>
  )
}
        ...
        <Suspense fallback={null}>
          ...
          {enablePost && (
            <EffectComposer multisampling={0}>
              <Bloom intensity={0.55} luminanceThreshold={0.12} luminanceSmoothing={0.22} blendFunction={BlendFunction.SCREEN} />
